...
interface IMeuModuloList extends IDefaultListProps {
	remove: (doc: IMeuModulo) => void;
	viewComplexTable: boolean;
	setViewComplexTable: (_enable: boolean) => void;
	meuModulos: IMeuModulo[];
	setFilter: (newFilter: Object) => void;
	clearFilter: () => void;
}

const MeuModuloList = (props: IMeuModuloList) => {
	const {
		meuModulos,
		navigate,
		remove,
	...
	const idMeuModulo = shortid.generate();

	const onClick = (_event: React.SyntheticEvent, id: string) => {
		navigate('/meuModulo/view/' + id);
	};
	...
	const callRemove = (doc: IMeuModulo) => {
		...
		<PageLayout title={'Lista de Exemplos'} actions={[]}>
			...
			{!isMobile && viewComplexTable && (
				<ComplexTable
					...
					onRowClick={(row) => navigate('/meuModulo/view/' + row.id)}
					searchPlaceholder={'Pesquisar exemplo'}
					onDelete={callRemove}
					onEdit={(row) => navigate('/meuModulo/edit/' + row._id)}
					toolbar={{
						selectColumns: true,
			...
			<RenderComPermissao recursos={[Recurso.EXAMPLE_CREATE]}>
				<div
					...
					<Fab id={'add'} onClick={() => navigate(`/meuModulo/create/${idMeuModulo}`)} color={'primary'}>
						<Add />
					</Fab>
				</div>
			</RenderComPermissao>
